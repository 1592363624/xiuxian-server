...
- **权限管理**：RBAC 角色权限系统，支持细粒度权限控制；PLAYER 角色自动包含所有 `game.*` 和 `qq.*` 权限，新增权限码无需逐个加入角色定义
- **QQ 机器人**：OneBot 协议集成，支持私聊和群聊指令操作
- **指令即路由**：一个 Command 子类同时定义 OneBot 指令 + REST API 端点，HTTP 路由由 UnifiedRestResource 自动注册，无需多处修改
...
- **添加游戏指令**：在 `onebot/command/` 下新建 `XxxCommand extends Command` 即可自动注册为 OneBot 指令；覆写 `getRestEndpoints()` 可同时注册 HTTP API，无需修改 `GameResource`
- **添加权限码**：在 `PermissionCode` enum 加一行即可，`game.*` 和 `qq.*` 前缀自动授予 PLAYER 角色
- **添加分类**：在 `Command.CATEGORY_ORDER` 加一行即可控制 `/帮助` 菜单排序
